...
COLUMN_SYNONYMS = {
    ...
    "p1_type": ["p1type", "firsttype", "qtype", "1차공정", "quenchingtype", "solutionheattreatment", "solutiontreatment", "solution", "sht", "고용화", "용체화"],
...
def _parse_dca_material_certificate(text, file_name=""):
    ...
    try:
        ...
        proc_alias = {
            ...
            "Solution Heat Treatment": ("p1", "Solution Heat Treatment", "수냉(WQ)"),
            "Solution": ("p1", "Solution Heat Treatment", "수냉(WQ)"),
            "SHT": ("p1", "Solution Heat Treatment", "수냉(WQ)"),
            "Tempering": ("p2", "Tempering", "공냉(AC)"),
            "PWHT": ("p3", "PWHT", "노냉(FC)"),
        }
        for line in lines[ht_idx + 2: ht_idx + 12]:
            pm = re.match(r"^(Preliminary|Quenching|Solution\s+Heat\s+Treatment|Solution|SHT|Tempering|PWHT):\s*(.*)", line, flags=re.I)
            if not pm:
                continue
            pname_raw = re.sub(r"\s+", " ", pm.group(1).strip())
            if pname_raw.upper() == "PWHT":
                pname = "PWHT"
            elif pname_raw.upper() == "SHT":
                pname = "SHT"
            elif re.search(r"solution", pname_raw, flags=re.I):
                pname = "Solution Heat Treatment"
            else:
...
with tab_predict:
    ...
    with c1:
        ...
        p1_type = st.selectbox("공정 종류", ["Quenching", "Solution Heat Treatment", "Normalizing", "Annealing"], key="p1_type_select")
...
with tab_measured:
    ...
    with hp1:
        a_p1_type = st.selectbox("1차 공정", ["Quenching", "Solution Heat Treatment", "Normalizing", "Annealing"], key="actual_p1_type")
    ...
    if uploaded_file is not None:
        try:
            ...
            if parsed_df.empty:
                st.warning("자동 추출 가능한 행을 찾지 못했습니다. PDF는 텍스트 선택형/이미지형 모두 지원하지만, 이미지 PDF는 Tesseract OCR 설치가 필요합니다. 대창 Material Certificate 양식은 전용 파서로 4개 양각/열처리 그룹을 우선 인식합니다.")
            else:
                default_meta = {
